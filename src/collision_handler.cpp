...
void CollisionHandler::check_friendly_sensor_collisions() {
    ...
    for (auto friendly_sensor : friendly_sensors_) {
        std::vector<Hitbox> friendly_sensor_hitboxes = friendly_sensor.second->get_hitboxes();
		int hitbox_number = 0;
        for (auto friendly_hitbox : friendly_sensor_hitboxes) {
            
            
            for (auto enemy_sensor : enemy_sensors_) {
                for (auto enemy_hitbox : enemy_sensor.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(friendly_hitbox, enemy_hitbox)) {
                        friendly_sensor.second->notify_collision(enemy_sensor.second, hitbox_number);
                    }
                }
            }
            for (auto enemy_projectile : enemy_projectiles_) {
                for (auto enemy_hitbox : enemy_projectile.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(friendly_hitbox,enemy_hitbox)) {
                        friendly_sensor.second->notify_collision(enemy_projectile.second, hitbox_number);
                    }
                }
            }
            for (auto enemy_ship : enemy_ships_) {
                for (auto enemy_hitbox : enemy_ship.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(friendly_hitbox, enemy_hitbox)) {
                        friendly_sensor.second->notify_collision(enemy_ship.second, hitbox_number);
                    }
                }
            }
...
void CollisionHandler::check_enemy_sensor_collisions() {
    ...
    for (auto enemy_sensor : enemy_sensors_) {
        std::vector<Hitbox> enemy_sensor_hitboxes = enemy_sensor.second->get_hitboxes();
		int hitbox_number = 0;
        for (auto enemy_hitbox : enemy_sensor_hitboxes) {
            
            
            for (auto friendly_sensor : friendly_sensors_) {
                for (auto friendly_sensor_hitbox : friendly_sensor.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(enemy_hitbox, friendly_sensor_hitbox)) {
                        enemy_sensor.second->notify_collision(friendly_sensor.second, hitbox_number);
                    }
                }
            }
            for (auto friendly_projectile : friendly_projectiles_) {
                for (auto friendly_hitbox : friendly_projectile.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(enemy_hitbox, friendly_hitbox)) {
                        enemy_sensor.second->notify_collision(friendly_projectile.second, hitbox_number);
                    }
                }
            }
            for (auto friendly_ship : friendly_ships_) {
                for (auto friendly_hitbox : friendly_ship.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(enemy_hitbox, friendly_hitbox)) {
                        enemy_sensor.second->notify_collision(friendly_ship.second, hitbox_number);
                    }
                }
            }
            hitbox_number++;
...
void CollisionHandler::check_friendly_projectile_collisions() {
    ...
    for (auto friendly_projectile : friendly_projectiles_) {
        std::vector<Hitbox> friendly_projectile_hitboxes = friendly_projectile.second->get_hitboxes();
		int hitbox_number = 0;
            ...
            for (auto enemy_projectile : enemy_projectiles_) {
                for (auto enemy_hitbox : enemy_projectile.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(friendly_projectile_hitbox, enemy_hitbox)) {
                        friendly_projectile.second->notify_collision(enemy_projectile.second, hitbox_number);
                    }
                }
            }
            for (auto enemy_ship : enemy_ships_) {
                for (auto enemy_hitbox : enemy_ship.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(friendly_projectile_hitbox, enemy_hitbox)) {
                        friendly_projectile.second->notify_collision(enemy_ship.second, hitbox_number);
                    }
                }
            }
...
void CollisionHandler::check_enemy_projectile_collisions() {
	...
	for (auto enemy_projectile : enemy_projectiles_) {
        std::vector<Hitbox> enemy_projectile_hitboxes = enemy_projectile.second->get_hitboxes();
		int hitbox_number = 0;
            ...
            for (auto friendly_projectile : friendly_projectiles_) {
                for (auto friendly_hitbox : friendly_projectile.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(enemy_projectile_hitbox, friendly_hitbox)) {
                        enemy_projectile.second->notify_collision(friendly_projectile.second, hitbox_number);
                    }
                }
            }
            for (auto friendly_ship : friendly_ships_) {
                for (auto friendly_hitbox : friendly_ship.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(enemy_projectile_hitbox, friendly_hitbox)) {
                        enemy_projectile.second->notify_collision(friendly_ship.second, hitbox_number);
                    }
                }
            }
...
void CollisionHandler::check_friendly_ship_collisions() {
 ...
 for (auto friendly_ship : friendly_ships_) {
        std::vector<Hitbox> friendly_ship_hitboxes = friendly_ship.second->get_hitboxes();
		int hitbox_number = 0;
            ...
            for (auto enemy_ship : enemy_ships_) {
                for (auto enemy_hitbox : enemy_ship.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(friendly_ship_hitbox, enemy_hitbox)) {
                        friendly_ship.second->notify_collision(enemy_ship.second, hitbox_number);
                    }
                }
            }
...
void CollisionHandler::check_enemy_ship_collisions() {
    ...
    for (auto enemy_ship : enemy_ships_) {
        std::vector<Hitbox> enemy_ship_hitboxes = enemy_ship.second->get_hitboxes();
		int hitbox_number = 0;
            ...
            for (auto friendly_ship : friendly_ships_) {
                for (auto friendly_hitbox : friendly_ship.second->get_hitboxes()) {
                    if (collision_detector_.is_overlapping(enemy_ship_hitbox, friendly_hitbox)) {
                        enemy_ship.second->notify_collision(friendly_ship.second, hitbox_number);
                    }
                }
            }
